...
CC=$CC CXX=$CXX CFLAGS=$CFLAGS CXXFLAGS=$CXXFLAGS MAX_JOBS=$(nproc) USE_ITT=0 USE_FBGEMM=0 BUILD_BINARY=1 USE_STATIC_MKL=1 \
    ...
    BUILD_SHARED_LIBS=OFF USE_OPENMP=0 USE_MKLDNN=0 USE_CUDA=OFF BUILD_CUSTOM_PROTOBUF=OFF \
...
export PKG=$(find /pytorch/dist -name torch*.whl)
python3 -m pip install --upgrade --no-deps --force-reinstall ${PKG}
